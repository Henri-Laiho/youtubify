from conf import downloaded_audio_folder as download_folder
import os

from ytdownload import get_file_extension, get_file_extension_if_exists, is_file_on_disk
...
class Track:
    def __init__(self, track_json):
        ...
        self.release = self.album['release_date']
        self.art_urls = self.album['images']
    ...
    def update_file_extension(self):
        self.extension = get_file_extension_if_exists(self.filename, download_folder)

    def update_filename_on_disk(self, old_filename):
        if not self.extension: self.update_file_extension()
